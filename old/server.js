...
app.use(
  morgan(
    '[:date[iso]] :ip :user :method :url :status :body :response-time ms',
...
cron.schedule('*/1 * * * *', () => {
  ...
  business_sql.query(
    `SELECT * FROM applications WHERE currentStatus = 'Approved' and bookedDate is NULL;`,
    function (err, res) {
      if (err) {
          ...
          business_sql.query(
            ...
            function (err, res) {
              if (err) {
                ...
                business_sql.query(
                  `UPDATE applications SET bookedDate = '${booked}' WHERE id = ${record.id};`
                );
                ...
                business_sql.query(
                  ...
                  function (err, res) {
                    if (err) {
                      console.log(
                        'INSERT INTO business_sql accounts error: ',
                        err
                      );
                      //result(null, err);
                    } else {
...
cron.schedule('*/1 * * * *', () => {
  ...
  business_sql.query(
    `UPDATE cases SET currentStatus = 'Open' WHERE currentStatus = 'Locked' AND lockedDatetime < (now() - interval 30 minute);`,
    function (err, res) {
      if (err) {
